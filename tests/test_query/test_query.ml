...
let parsed_query_out4 =
  ...
  ; out_params =
      ...
      ; { typ = Some "Name", "t"
        ; opt = false
        ; name = "name"
        ; of_string = "Name", "of_string"
        ; to_string = "Name", "to_string"
        }
      ; { typ = Some "Phone", "t"
        ; opt = true
        ; name = "phone"
        ; of_string = "Phone", "of_string"
...
let query_inout =
  "SELECT @int64{id}, @string{name}, @string?{phone} FROM users WHERE id = %int64{id} OR \
   name = %string{name} OR PHONE = %string?{phone}"
;;
...
let parsed_query_list0 =
  ...
  ; in_params =
      [ { typ = None, "int"
        ...
        ; to_string = "Stdlib", "string_of_int"
        }
      ]
  ; out_params =
      [ { typ = None, "int"
